...
var FORM = (function( $ ){
    ...
    var defaultFieldOptions = {
        ...
        // ------------------------------------------------------------
        _fieldsStringSelector = 'input:not([type="reset"]):not([type="submit"]), select, textarea',
        ...
        _isFieldChecked = function( $field, fieldOptions ){
            ...
            if( $field.is('[type="radio"]') ){
                ...
                return ($fieldChecked.length > 0 && $fieldChecked.val().trim().length > 0 && validReqFrom);
        ...
        _isValid = function( $field, addedValidations ){
            ...
            return (
                ...
                validationRules[fieldType]( fieldValue, addedValidations ) : 
                fieldValue.length > 0
        ...
        getFormJSON = function( $form ){
            ...
            $form.find( _fieldsStringSelector ).not('[data-exclude-json]').each(function(){
                var $field = $(this),
        ...
        isValidField = function( $field, fieldOptions ){
            ...
            if( isValidateIfFilled && !isValidValue ){
                ...
                if( fieldType === 'radio' ){
                    ...
                    if( $field.is('[data-required-from]') ){
                        ...
                        var $reqMore = $( $field.data('required-from') + '[type="radio"]');
                        if( isValidValue ){
                            $reqMore.prop('checked', true);
                            $field.prop('required', true);
                        }
                        if( $reqMore.filter(':checked').length === 0 ){
                            
                            options.skipUIfeedback = true;
                            isValid = true;
                            finalValidation = false;
                            
                        }
                        
                    } else if( $field.is('[data-equal-to]') ){
                    ...
                    isValid = (finalValidation ? _isValid( $field, extraValidations ) : isValid);
        ...
        isValidForm = function( $form, options ){
            ...
            $form.find( _fieldsStringSelector ).each(function(idx, elem){
                ...
                if( $field.is('[required], [data-validate-if-filled]') ){
        ...
        submitAjaxForm = function( $form, options ){
            ...
            if( typeof options.formOptions.beforeSend === 'function' ){
                ...
                if( Object.prototype.toString.call(beforeSendFn) === '[object Object]' ){
                    formData = beforeSendFn.formData || formData;
            ...
            if( $form.is('[data-ajax-settings]') ){
                ...
                try {
                    ...
                    console.log('data-ajax-settings specified for ' + $form.attr('name') + ' form is not a valid JSON object!');
                    return false;
                }
            }
        ...
        init = function( $form, options ){            
            ...
            $forms.each(function(){
                ...
                $thisForm.find( _fieldsStringSelector ).not('[type="radio"], [type="checkbox"]').each(function(){
                    ...
                    if( $field.is('[required], [data-required-from], [data-validate-if-filled]') && $field.val() ){
                ...
                $thisForm.on(fieldOptions.validateOnEvents, _fieldsStringSelector, function( event ){
                    ...
                    if(
                        $this.is('[required], [data-required-from], [data-validate-if-filled]') &&
                        (
