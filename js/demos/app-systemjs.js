    ...
    return {
        execute: (function () {
            ...
            const addClass = (element, cssClasses) => {
              ...
              isNodeList = nodeList => NodeList.prototype.isPrototypeOf(nodeList),
              removeClass = (element, cssClasses) => {
                element.classList.remove(...cssClasses.split(" "));
              },
              isDOMNode = node => Element.prototype.isPrototypeOf(node),
              customEvents_field = {
                ...
                formOptions: {
                  ...
                  getFormData: function ($filteredFields, trimValues) {
                    ...
                    return $filteredFields.forEach($field => {
                      const isCheckbox = "checkbox" === $field.type,
              ...
              validationRules = {
                ...
                checkbox: function (value, $field) {
                  ...
                  return $dataChecks ? function ($field) {
                    const attrValue = JSON.parse($field.dataset.checks),
                      checkedLength = $field.form.querySelectorAll(`[name="${$field.name}"]:checked`).length,
                ...
                exactLength: function (value, $field) {
                  const valueLength = value.length,
                    exactLength = 1 * $field.dataset.exactLength,
                    obj = {
                ...
                file: function (value, $field, fieldOptions) {
                  const maxFileSize = 1 * ($field.getAttribute("data-max-file-size") || fieldOptions.maxFileSize),
                ...
                length: function (value, $field) {
                  const valueL = value.length,
                    attrValue = JSON.parse($field.dataset.length),
              ...
              keypressMaxlength = function (event) {
                ...
                if ($field.matches("[maxlength]")) {
                  const maxLength = 1 * $field.maxLength,
            ...
            function submit(event) {
              ...
              removeClass($form, `${formCssClasses.ajaxComplete} ${formCssClasses.ajaxError} ${formCssClasses.ajaxSuccess}`), addClass($form, formCssClasses.submit), instance.validateForm().then(data => {
            ...
            const groupValidationEnd = function (event) {
              ...
              validation = function (event) {
                const isChangeEvent = "change" === event.type,
                  $field = event.target,
                  self = $field.form.formjs;
                if ($field.matches(fieldsStringSelector)) {
                  ...
                  (isFieldForChangeEventBoolean && isChangeEvent || !isFieldForChangeEventBoolean && (!isChangeEvent || hasOnlyChangeEvent)) && self.validateField($field).then(() => {
                    const type = $field.type,
                      $relatedEqualTo = $field.form.querySelector(`[data-equal-to="${$field.name}"]`);
              ...
              validationEnd = function (event) {
                const eventDetail = event.detail,
                  ...
                  fieldOptions = mergeObjects({}, $field.form.formjs.options.fieldOptions, dataFieldOptions),
                  $container = $field.closest(fieldOptions.questionContainer),
                  isReqFrom = $field.matches("[data-required-from]"),
                  $reqMore = document.querySelector($field.dataset.requiredFrom);
                if ($container && !fieldOptions.skipUIfeedback) if (eventDetail.result) {
                  ...
                  removeClass($container, `${fieldOptions.cssClasses.valid} ${errorClassToRemove}`), addClass($container, errorClasses);
                }
              };
            async function checkFieldValidity($field, fieldOptions, validationRules, validationErrors) {
              ...
              const dataObj = (await runFunctionsSequence({
                ...
                needsValidation = $field.required || $field.matches("[data-validate-if-filled]") && isValidValue;
              ...
              const validationResult = needsValidation ? await async function ($field, fieldOptions, validationRules, validationErrors) {
                  const fieldValue = $field.value,
                    ...
                    isCheckboxOrRadio = ["checkbox", "radio"].includes($field.type),
                    hasSelectedInput = $field.form.querySelectorAll(`[name="${$field.name}"]:checked`).length > 0;
                  if (!isCheckboxOrRadio && !obj.result || isCheckboxOrRadio && !hasSelectedInput) return obj.result = !1, obj.errors = {
                    empty: !0
                  }, Promise.resolve(obj);
                  const validationMethods = Array.from($field.attributes).reduce((accList, attr) => {
                      const attrName = toCamelCase(attr.name.replace("data-", "")),
                        attrValue = toCamelCase(attr.value),
                        isAttrValueWithFn = ["type", "subtype"].includes(attrName) && validationRules[attrValue],
                ...
                $container = fieldOptions.questionContainer && validationResult.$field.closest(fieldOptions.questionContainer);
            ...
            async function checkFieldsValidity($fields, fieldOptions, validationRules, validationErrors) {
              ...
              const $fieldsList = getUniqueFields($fields),
                fieldsValidity = await Promise.all($fieldsList.map($field => {
            ...
            class Form {
              constructor(form, optionsObj) {
                const argsL = arguments.length,
                  checkFormElem = (form => {
                    const formIsString = "string" == typeof form,
                      isFormSelector = formIsString && isDOMNode(document.querySelector(form)) && "form" === document.querySelector(form).tagName.toLowerCase();
                    return {
                ...
                ["beforeValidation", "beforeSend", "getFormData"].forEach(cbName => {
                  ...
                  fieldOptions.strictHtmlValidation && ($form.addEventListener("keypress", keypressMaxlength, !1), $form.addEventListener("input", dataTypeNumber, !1)), fieldOptions.preventPasteFields && $form.querySelectorAll(fieldOptions.preventPasteFields).length && $form.addEventListener("paste", pastePrevent, !1), fieldOptions.validateOnEvents.split(" ").forEach(eventName => {
                ...
                const initOptions = {};
                self.options.formOptions.onInitCheckFilled && (initOptions.detail = self.validateFilledFields().catch(fields => {})), dispatchCustomEvent(self.$form, customEvents_form.init, initOptions);
              }
              destroy() {
                !function ($form, options) {
                  options.fieldOptions.strictHtmlValidation && ($form.removeEventListener("keypress", keypressMaxlength, !1), $form.removeEventListener("input", dataTypeNumber, !1)), options.fieldOptions.preventPasteFields && $form.removeEventListener("paste", pastePrevent, !1), options.formOptions.handleSubmit && $form.removeEventListener("submit", submit), options.fieldOptions.validateOnEvents.split(" ").forEach(eventName => {
              ...
              async validateField(field, fieldOptions) {
                const self = this,
                  $form = self.$form,
                  $field = "string" == typeof field ? $form.querySelector(field) : field;
                fieldOptions = mergeObjects({}, self.options.fieldOptions, fieldOptions);
                const fieldValidity = await checkFieldValidity($field, fieldOptions, self.validationRules, self.validationErrors);
                if (dispatchCustomEvent(fieldValidity.$field, customEvents_field.validation, {
              ...
              async validateFieldsGroup() {
                ...
                const groups = this.options.formOptions.groups;
              ...
              async validateFilledFields(fieldOptions) {
                const $filledFields = ($form = this.$form, getUniqueFields($form.querySelectorAll(fieldsStringSelector)).map($field => {
                  const name = $field.name,
                    type = $field.type,
                    isCheckboxOrRadio = ["checkbox", "radio"].includes(type),
                    fieldChecked = $form.querySelector(`[name="${name}"]:checked`),
                    isReqFrom = $field.matches("[data-required-from]"),
                    $reqMore = isReqFrom ? $form.querySelector($field.dataset.requiredFrom) : null;
              ...
              async validateForm(fieldOptions) {
                ...
                const $form = self.$form,
                  ...
                  formVaidity = await checkFieldsValidity($fields, fieldOptions, self.validationRules, self.validationErrors);
            ...
            Form.prototype.options = options, Form.prototype.validationErrors = {}, Form.prototype.validationRules = validationRules, Form.prototype.version = "6.0.0";
