  ...
  return {
    execute: function () {
      ...
      var addClass = function addClass(element, cssClasses) {
        ...
        var splitChar = ".";
        return -1 === string.indexOf(splitChar) && (string.indexOf("-") >= 0 ? splitChar = "-" : string.indexOf("/") >= 0 && (splitChar = "/")), splitChar;
      },
        ...
        return Array.from(nodeList).filter(function (fieldEl) {
          var name = fieldEl.name,
              type = fieldEl.type;
          return (name !== currentFieldName || type !== currentFieldType) && (fieldEl.matches("[data-required-from]") || (currentFieldName = name, currentFieldType = type), !0);
        });
      },
          getValidateFieldDefault = function getValidateFieldDefault(obj) {
        return mergeObjects({}, {
          result: !1,
          fieldEl: null
        }, obj);
      },
          getValidateFormDefault = function getValidateFormDefault(obj) {
        return mergeObjects({}, {
          result: !0,
          fields: []
        }, obj);
        ...
        formOptions: {
          getFormData: function getFormData(filteredFields) {
            ...
            return filteredFields.forEach(function (fieldEl) {
              var isCheckbox = "checkbox" === fieldEl.type,
        ...
        checkbox: function checkbox(data) {
          var dataChecksEl = data.fieldEl.closest("form").querySelector('[name="' + data.fieldEl.name + '"][data-checks]'),
              obj = {
        ...
        equalTo: function equalTo(data) {
          var fieldEl = data.fieldEl,
              checkFromEl = fieldEl.closest("form").querySelector('[name="' + fieldEl.getAttribute("data-equal-to") + '"]'),
              obj = {
        ...
        file: function file(data) {
          var fieldEl = data.fieldEl,
              maxFileSize = 1 * (fieldEl.getAttribute("data-max-file-size") || data.fieldOptions.maxFileSize),
          ...
          return filesList.forEach(function (file) {
            ...
            !exceedMaxFileSize && isAcceptedFileType || (obj.result = !1, void 0 === obj.errors && (obj.errors = {}), obj.errors.file = !0, exceedMaxFileSize && (obj.errors.maxFileSize = !0), isAcceptedFileType || (obj.errors.acceptedFileType = !0));
          }), obj;
        },
        ...
        max: function max(data) {
          ...
          if (isDate) {
            ...
            value = 2 === value.indexOf(splitChar) ? value.split(splitChar).reverse() : value.split(splitChar), value = value.join(""), maxVal = maxVal.split("-").join("");
          }
        ...
        min: function min(data) {
          ...
          if (isDate) {
            var splitChar = getSplitChar(value);
            value = 2 === value.indexOf(splitChar) ? value.split(splitChar).reverse() : value.split(splitChar), value = value.join(""), minVal = minVal.split("-").join("");
          }
      ...
      var listenerCallbacks_dataTypeNumber = function listenerCallbacks_dataTypeNumber(event) {
        ...
        if (fieldEl.matches('[data-type="number"]')) {
          ...
          listenerCallbacks_keypressMaxlength = function listenerCallbacks_keypressMaxlength(event) {
        var fieldEl = event.target;

        if (fieldEl.matches("[maxlength]")) {
          var maxLength = 1 * fieldEl.maxLength,
              keyPressed = event.which || event.keyCode,
              allowedKeys = [8, 37, 38, 39, 46];
          if (fieldEl.value.length >= maxLength && -1 === allowedKeys.indexOf(keyPressed)) return !1;
        }
      },
          listenerCallbacks_pastePrevent = function listenerCallbacks_pastePrevent(event) {
        var fieldEl = event.target;
        ...
        fieldEl.matches(fieldOptions.preventPasteFields) && event.preventDefault();
          ...
          listenerCallbacks_submit = function listenerCallbacks_submit(event) {
        submit(event);
      },
          listenerCallbacks_validation = function listenerCallbacks_validation(event) {
        ...
        if (fieldEl.matches(fieldsStringSelector)) {
          ...
          if (isRadio && "change" === eventName) {
            var findReqMoreEl = isReqMore ? fieldEl : self.formEl.querySelector('[name="' + fieldEl.name + '"][data-require-more]'),
                findReqFromEl = null !== findReqMoreEl ? self.formEl.querySelector('[data-required-from="#' + findReqMoreEl.id + '"]') : null;
            isReqMore ? null !== findReqFromEl && (findReqFromEl.required = fieldEl.required, self.options.fieldOptions.focusOnRelated && findReqFromEl.focus()) : null !== findReqMoreEl && null !== findReqFromEl && (findReqFromEl.required = findReqMoreEl.required && findReqMoreEl.checked, findReqFromEl.value = "");
          }
          ...
          listenerCallbacks_validationEnd = function listenerCallbacks_validationEnd(event) {
        var fieldsArray = event.data.fieldEl ? [event.data] : event.data.fields,
        ...
        fieldsArray.forEach(function (obj) {
          ...
          if (fieldEl.matches(fieldsStringSelector)) {
            ...
            if (null !== containerEl && removeClass(containerEl, options.cssClasses.pending), null !== containerEl && !options.skipUIfeedback) if (obj.result) {
              ...
              var extraErrorClass = options.cssClasses.errorRule,
                  isChecks = fieldEl.matches("[data-checks]"),
      ...
      function formStartup(formEl, options) {
        ...
        fieldOptions.handleValidation && (fieldOptions.strictHtmlValidation && (formEl.addEventListener("keypress", listenerCallbacks_keypressMaxlength, !1), formEl.addEventListener("input", listenerCallbacks_dataTypeNumber, !1)), fieldOptions.preventPasteFields && formEl.querySelectorAll(fieldOptions.preventPasteFields).length && formEl.addEventListener("paste", listenerCallbacks_pastePrevent, !1), fieldOptions.validateOnEvents.split(" ").forEach(function (eventName) {
          var useCapturing = "blur" === eventName;
          formEl.addEventListener(eventName, listenerCallbacks_validation, useCapturing);
        }), formEl.addEventListener(customEvents_field.validation, listenerCallbacks_validationEnd, !1)), formOptions.handleSubmit && (formEl.addEventListener("submit", listenerCallbacks_submit), formOptions.ajaxSubmit && (formEl.getAttribute("enctype") && (formOptions.ajaxOptions.headers["Content-Type"] = formEl.getAttribute("enctype")), formEl.getAttribute("method") && (formOptions.ajaxOptions.method = formEl.getAttribute("method").toUpperCase()), formEl.getAttribute("action") && (formOptions.ajaxOptions.url = formEl.getAttribute("action"))));
      }
      ...
      function checkFieldValidity(fieldEl, fieldOptions, validationRules, validationErrors) {
        ...
        var isValidValue = fieldEl.value.trim().length > 0,
        ...
        return runFunctionsSequence(rfsObject).then(function (data) {
          ...
          return new Promise(function (resolve) {
            !isRequired && !isValidateIfFilled && !isReqFrom || isValidateIfFilled && !isValidValue || isReqFrom && !isRequired ? (dataObj.result = !0, resolve(dataObj)) : resolve(function (fieldEl, fieldOptions, validationRules, validationErrors) {
              var fieldType = fieldEl.matches("[data-subtype]") ? toCamelCase(fieldEl.getAttribute("data-subtype")) : fieldEl.type,
                ...
                return a.name < b.name;
                  ...
                  attrValidations = [];
              var attrValidationsResult = isValidValue,
                  obj = getValidateFieldDefault({
                result: isValidValue,
                fieldEl: fieldEl
              });
              return obj.result ? (fieldAttributes.forEach(function (attr) {
                var attrName = toCamelCase(attr.name.replace("data-", "")),
                    ...
                    isAttrValueWithFn = "type" === attrName && "function" == typeof validationRulesAttributes[attrValue],
                    isAttrNameWithFn = "function" == typeof validationRulesAttributes[attrName];
      ...
      function checkFormValidity(formEl, fieldOptions, validationRules, validationErrors) {
        ...
        var obj = getValidateFormDefault(),
        ...
        _createClass(Form, [{
          ...
          value: function destroy() {
            !function (formEl, options) {
              options.fieldOptions.strictHtmlValidation && (formEl.removeEventListener("keypress", listenerCallbacks_keypressMaxlength, !1), formEl.removeEventListener("input", listenerCallbacks_dataTypeNumber, !1)), options.fieldOptions.preventPasteFields && formEl.removeEventListener("paste", listenerCallbacks_pastePrevent, !1), options.formOptions.handleSubmit && formEl.removeEventListener("submit", listenerCallbacks_submit), options.fieldOptions.validateOnEvents.split(" ").forEach(function (eventName) {
                var useCapturing = "blur" === eventName;
                formEl.removeEventListener(eventName, listenerCallbacks_validation, useCapturing);
              }), formEl.removeEventListener(customEvents_field.validation, listenerCallbacks_validationEnd, !1), delete formEl.formjs;
            ...
            return checkFieldValidity(fieldEl, fieldOptions, this.validationRules, this.validationErrors).then(function (obj) {
              return new Promise(function (resolve) {
                obj.fieldEl && (dispatchCustomEvent(obj.fieldEl, customEvents_field.validation, obj, {
                  bubbles: !1
                }), dispatchCustomEvent(formEl, customEvents_field.validation, obj), fieldOptions.onValidationCheckAll && obj.result ? (fieldOptions.skipUIfeedback = !0, resolve(checkFormValidity(formEl, fieldOptions, _this.validationRules, _this.validationErrors, fieldEl).then(function (dataForm) {
          ...
          value: function validateForm(fieldOptions) {
            ...
            return checkFormValidity(formEl, fieldOptions, this.validationRules, this.validationErrors).then(function (data) {
              ...
              return formEl.classList[clMethodName](_this2.options.formOptions.cssClasses.valid), listenerCallbacks_validationEnd({
                data: data
      ...
      Form.prototype.isInitialized = !1, Form.prototype.options = options, Form.prototype.validationErrors = {
        email: function email(string) {
          ...
          if (-1 === string.indexOf("@")) obj.missingAtChar = !0;else {
            ...
            if (0 === splitAt_at[0].length && (obj.missingUserName = !0), 0 === splitAt_at[1].length) obj.missingDomain = !0, obj.missingExtensionDot = !0, obj.missingExtension = !0;else if (-1 === splitAt_at[1].indexOf(".")) obj.missingExtensionDot = !0, obj.missingExtension = !0;else {
